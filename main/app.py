...
def customCallback(client, userdata, message):
    ...
    conn=sqlite3.connect('sesnordata.db')
    c=conn.cursor()

    c.execute("""INSERT INTO SensorReadings (Temperature,
        Humidity, CurrentDate, CurrentTime, FirmwareVersion) VALUES((?), (?), date('now'),
        time('now'), (?))""", (dhtreadings_json['Temperature'],
        dhtreadings_json['Humidity'], '2') )
...
parser.add_argument("-id", "--clientId", action="store", dest="clientId", default="testClient",
                    help="Targeted client id")
parser.add_argument("-t", "--topic", action="store", dest="topic", default="temp/topic", help="Targeted topic")
